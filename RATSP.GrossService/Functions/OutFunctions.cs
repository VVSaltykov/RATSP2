...
public static class OutFunctions
{
        ...
        List<Fraction> fractions, DateOnly selectedDate)
    {
        ...
        ExcelHelper.SetCellValue(sheet, 6, 6,
            ...
            "Calibri", 12, (15.87, 18), decimalPlaces: 11);
        ...
        ExcelHelper.SetCellValue(sheet, 11, 18,
            ...
            "Calibri", 9, (16.27, 93), applyBorders: true, isBold: true,
            wrapText: true, textCenter: true);
        
        ExcelHelper.SetCellValue(sheet, 11, 19,
            "Вознаграждение администратора в руб.",
            "Calibri", 9, (10, 93), applyBorders: true, isBold: true,
            wrapText: true, textCenter: true);
        ...
        ExcelHelper.SetCellValue(sheet, 11, 21,
            "Вид страхования",
            "Calibri", 9, (16, 93), applyBorders: true, isBold: true,
            wrapText: true, textCenter: true);
    }
        ...
        Company company, List<Fraction> fractions, DateOnly selectedDate)
    {
        ...
        ExcelHelper.SetCellValue(sheet, rowCount, 0,
            ...
            "Calibri", 9, (5.27, 14.3),
            mergeRegion: (rowCount, rowCount, 0, 21), applyBorders: true, isBold: true, textCenter: true);

        rowCount++;
        
        foreach (var excelValues in excelValuesList)
        {
            insuranceType = InsuranceTypeHelper.GetShortType(excelValues.InsuranceType);
            ...
            if (DateOnly.Parse(excelValues.StartDate) >= companyFraction.Start && DateOnly.Parse(excelValues.StartDate) <= companyFraction.End) 
            {
                ...
                ExcelHelper.SetCellValue(sheet, rowCount, 1,
                    ...
                    "Calibri", 11, (25.27, 28.5), applyBorders: true, wrapText: true);
                ...
                ExcelHelper.SetCellValue(sheet, rowCount, 14,
                    Convert.ToDecimal(excelValues.NetPremium),
                    "Calibri", 11, (11, 28.5), applyBorders: true);

                premiumPercent = Math.Round(Convert.ToDecimal(excelValues.PremiumPercent) * 100, 2);
                ...
                reportingPeriodSum += Convert.ToDecimal(excelValues.NetPremium) * Convert.ToDecimal(excelValues.PaymentRate_ReturnRate)
                                                                                * Convert.ToDecimal(excelValues.PremiumPercent);
                ...
                ExcelHelper.SetCellValue(sheet, rowCount, 18,
                    ...
                    "Calibri", 11, (16.27, 28.5), applyBorders: true, wrapText: true);
                ...
                ExcelHelper.SetCellValue(sheet, rowCount, 20,
                    Convert.ToDecimal(excelValues.PaymentContract) * 100,
                    "Calibri", 11, (10, 28.5), applyBorders: true);
        ...
        ExcelHelper.SetCellValue(sheet, rowCount, 14,
            ...
            "Calibri", 10, (11, 14.3), isBold: true);
        
        ExcelHelper.SetCellValue(sheet, rowCount, 17,
            reportingPeriodSum,
            "Calibri", 10, (12.73, 14.3), isBold: true);
        
        ExcelHelper.SetCellValue(sheet, rowCount, 19,
            reportingPeriodAward,
            "Calibri", 10, (10, 14.3), isBold: true);
        ...
        foreach (var excelValues in excelValuesList)
        {
            ...
            if (_companyFraction != null)
            {
                if (DateOnly.Parse(excelValues.StartDate) < companyFraction.Start || DateOnly.Parse(excelValues.StartDate) > companyFraction.End)
                {
                    ...
                    {
                        ExcelHelper.SetCellValue(sheet, rowCount, 8,
                            Convert.ToDecimal(excelValues.InsuranceAmount_LiabilityLimit) * ((100 - _companyFraction.Value)/100),
                            "Calibri", 11, (15.73, 28.5), applyBorders: true);
                        ...
                        ExcelHelper.SetCellValue(sheet, rowCount, 18,
                            $"{number} / {excelValues.PaymentDate} / {_companyFraction.Value} / {excelValues.SanctionsRisk} / {excelValues.Comment}",
                            "Calibri", 11, (16.27, 28.5), applyBorders: true, wrapText: true);
